...
import {
  MincoexHostingSummarySection,
  MincoexHostingTiersSection,
  MincoexHostingSecuritySection,
} from "@/components/proposals/mincoex-hosting-content"

export default function MincoexHostingProposalPage() {
  const handlePrint = () => {
    ...
    <div className="bg-slate-100 flex justify-center items-start md:items-center p-0 md:p-4 print:p-0 font-sans min-h-screen">
      ...
      <div className="a4-container w-full md:w-[210mm] md:min-h-[297mm] bg-white md:shadow-xl relative overflow-hidden p-4 sm:p-6 md:p-[25mm] print:shadow-none print:p-[25mm]">
        ...
        <header className="text-center pb-5 border-b-2 border-blue-600 mb-6 md:mb-8 w-full">
          ...
          <h1 className="text-xl md:text-2xl font-bold text-slate-900">Propuesta de Hosting Institucional</h1>
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5 mb-6 md:mb-8">
          ...
          <article className="bg-slate-50 p-4 rounded-lg border-l-4 border-blue-600">
            ...
            <p className="text-slate-700 text-sm md:text-base">Fecha de emisión: {new Date().toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
        ...
        <section className="mt-8 md:mt-10 bg-slate-900 text-white rounded-2xl p-5 md:p-6">
          ...
          <div className="space-y-3 text-sm md:text-base text-slate-200 mb-4">
            <p>
              1. Seleccionar el plan que mejor se adapte a las necesidades del Ministerio.
            </p>
            <p>
              2. Emitir la orden de servicio o memorando interno para proceder con la activación.
            </p>
            <p>
              3. Coordinar con el equipo de TIC para la migración y apuntalamiento de DNS.
            </p>
          </div>
        </section>
