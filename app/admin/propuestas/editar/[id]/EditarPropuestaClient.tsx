...
import { updateProposal, Credential, CatalogCategory, Proposal, ProposalService, ProposalCollaborator, ProposalPayment } from "@/lib/actions"
...
interface EditarPropuestaClientProps {
  ...
  initialPayments: ProposalPayment[]
...
export default function EditarPropuestaClient({
  ...
  const [showCredentialsSection, setShowCredentialsSection] = useState(() => {
    return initialCredentials && initialCredentials.length > 0 && initialCredentials.some(c => c.email || c.password)
  })
    ...
    if (initialCredentials && initialCredentials.length > 0) {
      return initialCredentials.map(c => {
        ...
        return {
          ...
          notes: c.notes || "",
          fields,
          showAdvanced: !!(c.notes || fields.length > 0)
        }
      })
    }
    return [{ description: "Cuenta de Administración", email: "", password: "", notes: "", fields: [], showAdvanced: false }]
  })
  ...
  const [payments, setPayments] = useState<ProposalPayment[]>(() => {
    if (initialPayments && initialPayments.length > 0) {
      return initialPayments.map(p => ({
        ...
        amount: p.amount,
        payment_date: parseFriendlyDateToISO(p.payment_date || ""),
        payment_method: p.payment_method || "Binance",
        description: p.description || "Cuota de Contrato",
        status: p.status || "PENDIENTE",
        notes: p.notes || "",
        due_date: parseFriendlyDateToISO(p.due_date || "")
      }))
    }
    // Fallback a un pago único con el total de los servicios
    const total = initialServices.reduce((acc, curr) => acc + (curr.price || 0), 0)
    return [
      { description: "Pago Inicial / Único", amount: total, status: proposal.payment_status || "PENDIENTE", payment_date: parseFriendlyDateToISO(proposal.payment_date || ""), payment_method: proposal.payment_method || "Binance", notes: "", due_date: "" }
    ]
  })

  // Sincronizar el monto del primer pago con la suma total de los servicios si solo hay 1 cuota y no ha sido modificada manualmente
  useEffect(() => {
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-4">
              {payments.map((pay, index) => (
                <div key={index} className="p-4 border border-slate-900 rounded-lg bg-slate-900/10 space-y-3 relative animate-fade-in">
